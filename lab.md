...
<div class="lab-group">
  ...
  <ul>
    {% for m in group.members %}<li>{% if m.url %}<a href="{{ m.url }}">{{ m.name }}</a>{% else %}{{ m.name }}{% endif %}{% if m.note %} <span class="lab-note">({{ m.note | markdownify | remove: '<p>' | remove: '</p>' }})</span>{% endif %}</li>{% endfor %}
  </ul>
</div>
{% endfor %}
